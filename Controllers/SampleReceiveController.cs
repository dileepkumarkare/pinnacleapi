...
namespace Pinnacle.Controllers
{
    ...
    public class SampleReceiveController : ControllerBase
    {
        ...
        SampleReceiveModel model = new SampleReceiveModel();
        JwtStatus jwtStatus = new JwtStatus();
        [HttpPost]
        [Route("Save")]
        public IActionResult SaveSampleCollection(SampleReceiveEntity entity)
        {
            ...
            Ret res = tokenStatus.IstokenExpired == true ? tokenStatus : accessStatus.status ? model.Save(entity, jwtStatus) : accessStatus;
            return Ok(new
            {
                status = res.status,
